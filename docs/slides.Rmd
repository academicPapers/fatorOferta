...
```{r, echo = T}
fitVenda <- lm(log(VUvenda) ~ offset(.1*log(IndiceFiscal)) + offset(-.46*log(AreaConstruida)) +
                 PC, data = atibaia, subset = -60)
fitOferta <- lm(log(VUoferta) ~ offset(.1*log(IndiceFiscal)) + offset(-.46*log(AreaConstruida)) +
                 PC, data = atibaia, subset = -60)
```
...
```{r}
tab_model(fitVenda, fitOferta, digits = 3,
