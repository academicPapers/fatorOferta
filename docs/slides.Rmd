...
* most of the literature on the property market is written as though it was 
the very paradigm of a neo-classical perfect market—many buyers, many sellers,
homogeneous product, full information etc (Evans, 1995, p. 6; also see comment
by Watkins, 1998, p. 57).
...
problem akin to that of quantum physics where, in this case, market value cannot 
exist simultaneously with the need for valuation. Valuers are required when 
transaction costs are high but at this point market value does not exist. 
...
performing the important function of bringing together buyers and sellers who 
generally lack information. It is argued here that **valuers are not merely 
...
norms on the market, but this nevertheless is the effect of their actions. Once 
‘market value’ has been provided, market participants need not personally search 
...
* Somente funciona devido às amostras (de ofertas e de transações) terem a mesma
composição.
...
```{r}
library(readxl)
atibaia <- read_excel('data/Atibaia.xlsx')
...
Oferta <- augment(fitOferta, atibaia)
```
...
Venda <- augment(fitVenda, atibaia)
```
...
linearHypothesis(fitOferta, c("log(IndiceFiscal) = 0.10", 
                 "log(AreaConstruida) = -.375"))
```
]

.pull-right[
```{r}
linearHypothesis(fitVenda, c("log(IndiceFiscal) = 0.10", 
                 "log(AreaConstruida) = -.375"))
```
...
```{r, echo = T}
fitVenda <- lm(log(VUvenda) ~ offset(.1*log(IndiceFiscal)) + offset(-.375*log(AreaConstruida)) +
                 PC - 1, data = atibaia)
fitOferta <- lm(log(VUoferta) ~ offset(.1*log(IndiceFiscal)) + offset(-.375*log(AreaConstruida)) +
                 PC - 1, data = atibaia)
```
...
```{r}
tab_model(fitVenda, fitOferta, show.ci = FALSE, show.p = FALSE, show.r2 = FALSE)
```
